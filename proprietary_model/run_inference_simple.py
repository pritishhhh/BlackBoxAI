"""
Run proprietary-model inference against a CSV dataset.
"""
...
def _resolve_input_csv() -> str:
    ...
    return os.path.join(PROJECT_ROOT, "datasets", "Proprietary", "proprietary_data_test.csv")
...
def main() -> int:
    ...
    checkpoint_path = os.path.join(SCRIPT_DIR, "checkpoints", "proprietary_model.pt")

    print("=" * 80)
    print("PROPRIETARY MODEL - CSV INFERENCE")
    print("=" * 80)
    print()
    ...
    try:
        from inference import ProprietaryInference

        engine = ProprietaryInference(checkpoint_path=checkpoint_path)
    ...
    try:
        ...
        results_df = results_df[[column for column in column_order if column in results_df.columns]]
